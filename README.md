...
AimstarInAppMessaging.shared.screenView("Your Screen Name", on: self)
```
...
AimstarInAppMessaging.shared.screenView(
    "Your Screen Name",
    on: self,
...
`screenView` やモディファイアにカスタムパラメータを付与できます。
...
#### screenView(_:on:topAnchor:bottomAnchor:customParameter:)
...
```swift
@MainActor protocol AimstarInAppMessagingDelegate: AnyObject
```

このDelegateを実装したオブジェクトをAimstarInAppMessagingに設定することによって、以下のイベントを検知できます。すべてのメソッドに空のデフォルト実装が提供されているため、必要なメソッドのみ実装してください。

> **Note:** このプロトコルは `@MainActor` で宣言されています。`UIApplicationDelegate` / `UIViewController` などの UIKit クラスは暗黙的に MainActor になるため通常は問題ありません。非 MainActor クラスから実装する場合は、クラスまたはメソッドに `@MainActor` を明示してください。
...
```swift
enum InAppMessage {
    case popupModal(PopupModalAction)
    case popupBanner(PopupBannerAction)
    case embeddedBanner(EmbeddedBannerAction)
    case unsupported(rawType: String)
}
```

SDKが取り扱うメッセージの型です。デリゲートメソッドの引数として渡されます。`unsupported(rawType:)` は将来の action_type 追加に対応するフォールバックケースです。`switch` で網羅する場合は必ずこのケースも処理してください。

## AimstarError

```swift
enum AimstarError: Error
```

SDK 内部で発生するエラーを表すenum。`messageError(_:error:)` デリゲートメソッドの引数として渡されます。

| ケース | 関連値 | 説明 | リトライ可否 |
|---|---|---|---|
| `precondition(message:)` | `message: String` | 内部整合性エラー（例: 表示先 ViewController が見つからないなど） | 不可 |
| `clientError(statusCode:)` | `statusCode: Int?` | 4xx レスポンス | 不可 |
| `serverError(statusCode:)` | `statusCode: Int?` | 5xx レスポンス | 可能 |
| `networkError(error:)` | `error: any Error & Sendable` | URLSession エラー | 可能 |

## CustomParameterEntry / CustomParameterBuilder

```swift
public struct CustomParameterEntry
```

カスタムパラメータの1エントリを表す型。`putString` / `putInteger` 等のビルダー関数で生成します。public stored property は存在しません。

```swift
@resultBuilder public struct CustomParameterBuilder
```

`screenView` / `aimstarScreen` にカスタムパラメータを宣言的に渡すための `@resultBuilder`。同一キーが複数指定された場合は後勝ちで上書きされます。
...
埋め込みバナーを識別するためのコンポーネントID の型エイリアスです。

## IAMUIEmbeddedBanner (UIKit)

```swift
final class IAMUIEmbeddedBanner: UIView
```

UIKit用の埋め込みバナーコンポーネントです。

```swift
var componentId: ComponentID?
```

`componentId` は `Optional<ComponentID>`（= `String?`）です。設定する前（`nil` の状態）はシマーアニメーションが表示されます。`componentId` を設定するとそのコンポーネントIDに対応するコンテンツが表示されます。
